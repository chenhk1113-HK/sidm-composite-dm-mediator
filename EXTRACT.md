...
**Version 0.4-prelim + T89 · 2026-09-06 · ~1,250 words**

**Standing version: v0.4-prelim+T88E** (post-T89: Channel 25 Goldstein & Hill 2026 ΔN_eff<0.107 documented null + sidmkit/sidm-vdsigmas σ/m benchmark + 4 citation corrections). Channels: **22 effective** (T88.E v0.8 result preserved; Channel 25 is a documented null, no posterior shift); **Tests: 677 pass, 8 skip** (+15 from T89 Goldstein & Hill Channel 25 tests; was 662 / 8 post-T88.E). v0.8 standing posterior: log Z = −164.87 ± 0.084, σ/m = 0.06 cm²/g, m_χ ~ 770 GeV, m_φ ~ 453 MeV, a = +0.13, tension 0.60 (below 1.0). Drift-guard audit: 44/44 ALL CLEAR.

**⚠ Orthogonal-physics in practice (locked 2026-08-10, reaffirmed 2026-09-02 in T75; refined 2026-09-02 in T78; uncertainty band added 2026-09-02 in T79):** The project's headline σ/m = 0.06 cm²/g (post-T88.E v0.8; was 0.27 at v0.7) measures **σ_DM-DM** (self-scattering cross-section per unit mass, the SIDM observable). Direct-detection experiments (LZ, XENONnT, PandaX) measure **σ_DM-nucleon**. These are **theoretically linked** through kinetic mixing in the light-mediator SIDM model — but at the project's v0.8 posterior (ε ~ 10⁻³⁷, essentially unchanged from v0.7), σ_DM-nucleon is suppressed by **~50-80 orders of magnitude** relative to LZ sensitivity (the "70 orders" figure is a point-particle estimate applied to a composite model, with an interpolated LZ limit). So the link is **practically inert**. The project uses direct-detection constraints only as a sanity check (Channel 5), not as σ/m measurements. This stance is **non-negotiable** unless user explicitly overrides. See `MODEL_ASSUMPTIONS_AND_LIMITATIONS.md §0` and `v0.3-prelim/docs/T78_KINETIC_MIXING_LZ_LINK.md` + `v0.3-prelim/docs/T79_COMPOSITE_FORM_FACTOR_REMNANT.md`.

**📡 2026-09-01 LZ signal update (T77) + 2026-09-02 LZ paper (T80):** LZ announced a single high-energy event at **2.6σ global / 3.4σ local** significance (≈0.5% background probability). The actual LZ preprint (25 pages, NREFT + inelastic-DM framework) appeared 2026-09-02 with **best-fit Ls₁₀ at m_χ = 1000 GeV/c²** — very close to the project's v0.7 MAP m_χ ~ 770 GeV. **This does NOT change σ/m** (per the orthogonal-physics posture above). The 2.6σ global is below the 3σ threshold for updating Channel 5. The kinetic-mixing suppression (~50-80 orders per T79) holds even with the paper's detailed NREFT framework. KIV cron `080d2f590251` re-checks for PRL revision on 2026-11-01. See `v0.3-prelim/docs/T77_LZ_2026_09_UPDATE.md` and `v0.3-prelim/docs/T80_LZ_PAPER_UPDATE.md`.
...
- **Mediator quantum regime (Q&A 2026-08-25):** Per follow-up user question
  ...
  **classical regimes** (force-mediated, not wave-mediated). The mediator's
  ...
  correctly. See `v0.3-prelim/docs/PROJECT_FINDINGS.md` for the full sub-section.
...
> **Note (T89.2 doc-fix, 2026-09-06):** The 1.4–1.7 cm²/g figure was the **T21/T39-era (pre-v0.7)** standing number. It has been **superseded by the v0.7 → v0.8 re-run** (T88.E Euclid Q1 subhalo FORECAST, 2026-09-04). The current standing value is **σ/m = 0.06 cm²/g** at MAP (nlive=2000), as documented in `README.md` and `CURRENT.md`. The T21-era 1.4–1.7 cm²/g result is preserved in `v0.3-prelim/data/results/t41_mediator_mass_joint_fit_v0_4_historical.json` for reference.
