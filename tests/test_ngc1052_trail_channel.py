...
class TestNgc1052TrailChannel:
    ...
    def test_at_v0p3_prelim_map_is_near_neutral(self):
        ...
        With peak=0.5 cm^2/g restored, this is the model-consistent anchor.
        """
    ...
    def test_at_amuse_peak_would_penalize(self):
        """At the AMUSE-derived peak (sigma/m_0 ~ 3.07 cm^2/g, a ~ 0.0), the
        channel PENALIZES because the placeholder peak (0.5 cm^2/g) is restored
        and 3.07 is ~0.79 dex above it. Penalty = -0.5 * (0.79)^2 ~ -0.31.
        This test documents that the AMUSE peak creates tension with the
        model-consistent placeholder.
        """
        result = loglike_ngc1052_trail(sigma_m_0=3.07, a=0.0)
        assert math.isfinite(result)
        # At the AMUSE anchor, log L is penalized because the placeholder peak
        # (0.5) is restored. Penalty ~ -0.31.
        assert result < -0.2, f"AMUSE anchor not penalized enough: {result}"
        assert result > -0.5, f"AMUSE anchor penalized too strongly: {result}"
