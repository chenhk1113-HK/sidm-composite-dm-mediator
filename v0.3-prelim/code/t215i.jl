#!/usr/bin/env julia
# T215i — Sweep seeds to find one that reaches 60+ Myr
#
# Per Rv18.4.docx: After RNG seeding, if runs are deterministic per seed,
# different seeds give different t_max. Sweep seeds to characterize.
...
println("=" ^ 60)
println("T215i — seed sweep for 60+ Myr")
println("=" ^ 60)
...
# Save positions/velocities with seed 42 (deterministic IC selection)
...
# Try seed 42 (matches T215e original run)
t_end_Gyr = 0.07
...
params = CBEParams{1, SphericalGrid{1, Float64}, SelfGravity}(;
    ...
    snapshot_times,
    output_path="/tmp/t215i_seed42_output",
)

# === Run with seed 7 ===
Random.seed!(7)
println("\n\n=== RUN 2: Seed 7 ===")
println("Running with seed 7...")
...
try
    ...
    println("Run returned normally")
catch e
    println("Run THREW EXCEPTION:")
    showerror(stdout, e)
    println()
end
...
summary = Dict(
    ...
    "rng_seed_task_local" => 7,
    "timestamp" => string(now()),
)
out_path = "/mnt/c/Users/lamkuenai/projects/sidm-composite-dm-mediator/v0.3-prelim/data/results/t215i_seed7_summary.json"
