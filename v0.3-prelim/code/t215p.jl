#!/usr/bin/env julia
# T215p — 5x fresh-session density analysis
# Per Rv18.4 Round 4 rec #2: Run t215k with PER-RUN output paths so
# we get distinct snapshots, then analyze density profile of EACH run
# to verify whether the "qualitative signal robust" claim holds.

# This is the BATCH driver. Each invocation writes to a unique dir.
...
# Read run index from command line
run_idx = parse(Int, ARGS[1])
println("=== T215p Run $run_idx ===")
...
out_dir = "/tmp/t215p_run$(run_idx)_output"
...
Random.seed!(42)
println("Starting CBE_sim for run $run_idx...")
result = CBE_sim(params, pos_radial, vel_spherical)
println("Run $run_idx completed normally")
