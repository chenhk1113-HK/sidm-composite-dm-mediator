#!/usr/bin/env julia
# T215_gc_test — Test if Julia GC pressure is the issue.
# Strategy: Run with GC diagnostics enabled, try GC.disable() during critical loops.
...
println("=" ^ 60)
println("T215_gc_test — KiSS-SIDM with GC diagnostics")
...
idx = sort(rand(rng, 1:n_total, 5000))
...
GC.gc(true)
println("GC stats after setup: alloc=$(Base.gc_num().total_allocd/1e6) MB")
...
# Target: 50 Myr (28% of Balberg t_core) — try to push the boundary
t_end_Gyr = 0.05
...
params = CBEParams{1, SphericalGrid{1, Float64}, SelfGravity}(;
    ...
    snapshot_times,
    output_path="/tmp/t215_gc_output",
)
...
println("\nFinal GC stats: alloc=$(gc_stats.total_allocd/1e6) MB, pause=$(gc_stats.total_pause) ns")
...
out_path = "/mnt/c/Users/lamkuenai/projects/sidm-composite-dm-mediator/v0.3-prelim/data/results/t215_gc_summary.json"
