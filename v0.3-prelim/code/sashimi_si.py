...
try:
    ...
    # numexpr is optional optimization for sashimi_si (only used in 2 places).
    # Fall back to plain numpy.where() — ~2x slower but no install required.
    import numpy as _np_local
    class _NumExprStub:
        @staticmethod
        def evaluate(expr, **kwargs):
            # Inject numpy.where + arithmetic into eval namespace
            local_ns = dict(kwargs)
            local_ns['where'] = _np_local.where
...
class SIDM_parametric_model(SIDM_cross_section):
    ...
    def dVmaxSIDMdtt_numexpr_optimized(self, tt, Vmax_CDM):
        ...
        Local patch 2026-09-12: numexpr not installed; use np.where directly.
        """
        import numpy as _np
        tt_th = self.tt_th
        poly = (
            ...
            + 66.64 * tt * tt * tt
            - 94.35 * tt * tt * tt * tt
            + 63.539 * tt * tt * tt * tt * tt * tt
            - 21.924 * tt * tt * tt * tt * tt * tt * tt * tt
        )
        out = _np.where(tt <= tt_th, poly * Vmax_CDM, 0)
        return out
    ...
    def drmaxSIDMdtt_numexpr_optimized(self, tt, rmax_CDM):
        ...
        out = _np.where(tt <= tt_th, poly * rmax_CDM, 0)
        return out
