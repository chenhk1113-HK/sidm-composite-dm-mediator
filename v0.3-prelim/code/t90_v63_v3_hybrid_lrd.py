"""
T90.63 v3 -- Hybrid 10D joint fit with REFINED LRD channel (6 channels).

Replaces the v1 simplified Gaussian per-bin LRD likelihood with the
full UV luminosity function likelihood from t90_v63_lrd_channel_v3.
...
# Use v3 channel
from t90_v63_lrd_channel_v3 import (
    loglike_lrd_jiang2026_v3,
    LRD_UVLF_BINS,
    provenance_v3,
)
...
def loglike_lrd_for_hybrid(theta_10, enabled=True):
    ...
    try:
        ...
        return loglike_lrd_jiang2026_v3(sigma_m_log, enabled=enabled)
    except Exception:
        return -np.inf
...
def loglike_6ch_full(theta_10, lrd_enabled=True):
    ...
    Full 6-channel log-likelihood:
      ...
      - LRD channel from t90_v63_lrd_channel_v3
    """
...
def summarize(results, label="t90_v63_v3"):
    ...
    # Print
    print(f"[T90.63 v3] {label}: log Z = {log_z:.3f} +/- {log_z_err:.3f}")
...
def main():
    parser = argparse.ArgumentParser(description="T90.63 v3: 6-channel hybrid with refined LRD likelihood")
    ...
    parser.add_argument("--out", default="data/results/t90_v63_v3_hybrid_6ch_joint_posterior.json", help="Output file")
    ...
    label = f"T90.63 v3 hybrid 6ch (LRD {args.lrd}, KSFR {args.ksfr})"

    print(f"[T90.63 v3] Running {label}")
    ...
    print(f"[T90.63 v3] Results saved to {out_path}")
    print(f"[T90.63 v3] Wall time: {wall:.1f}s")
