#!/usr/bin/env julia
# T215_small — Try KiSS-SIDM with fewer particles (3000) to avoid the silent crash.
# Hypothesis: 10^4 particles triggers GC-related crash; 3000 may complete normally.
...
println("=" ^ 60)
println("T215_small — KiSS-SIDM with 3000 particles")
println("=" ^ 60)
...
println("Subsampled to $n_particles particles")
...
# Try t_end = 0.05 Gyr = 50 Myr (28% of Balberg t_core)
t_end_Gyr = 0.05
n_snapshots = 25  # every 2 Myr
...
params = CBEParams{1, SphericalGrid{1, Float64}, SelfGravity}(;
    ...
    Grav=SelfGravity,
    adaptive_grid=true,
    adaptive_grid_min_particles=32,  # lower threshold for fewer particles
    n_phys_per_tracer,
    ...
    snapshot_times,
    output_path="/tmp/t215_small_output",
)

println("Running with $n_particles particles, t_end=$t_end_Gyr Gyr")
println("  n_phys_per_tracer = $n_phys_per_tracer Msun")
...
out_path = "/mnt/c/Users/lamkuenai/projects/sidm-composite-dm-mediator/v0.3-prelim/data/results/t215_small_summary.json"
