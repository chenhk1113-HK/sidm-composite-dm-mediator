#!/usr/bin/env julia
# T215_safe — Patch KiSS-SIDM collision.jl in-place to use sqrt(max(0, x)).
# This is a local modification to the installed KiSS-SIDM source.
...
# Patch collision.jl to handle negative sq_diff from FP precision
# This is the same fix already applied in time_step.jl
collision_path = joinpath(dirname(pathof(DSMC)), "..", "src", "collision.jl")
collision_path = normpath(collision_path)
println("Collision.jl path: ", collision_path)

content = read(collision_path, String)

# Add max(0, x) protection to all vulnerable sqrt calls
old_pattern = "vmax = 5 * sqrt(v_rms^2 - sum(vbar .^ 2))"
new_pattern = "vmax = 5 * sqrt(max(zero(v_rms^2 - sum(vbar .^ 2)), v_rms^2 - sum(vbar .^ 2)))"

count = 0
new_content = content
while occursin(old_pattern, new_content)
    global new_content = replace(new_content, old_pattern => new_pattern, count=1)
    global count += 1
end

if count > 0
    write(collision_path, new_content)
    println("Patched $count occurrences of vmax = 5 * sqrt(...)")
else
    println("No occurrences found — already patched or different pattern")
end
...
println("=" ^ 60)
println("T215_safe — KiSS-SIDM with collision.jl patched")
println("=" ^ 60)
...
# Target: 100 Myr (57% of Balberg t_core) — push the boundary
t_end_Gyr = 0.10
...
params = CBEParams{1, SphericalGrid{1, Float64}, SelfGravity}(;
    ...
    snapshot_times,
    output_path="/tmp/t215_safe_output",
)

println("Running with $n_particles particles, t_end=$t_end_Gyr Gyr (100 Myr)")
...
out_path = "/mnt/c/Users/lamkuenai/projects/sidm-composite-dm-mediator/v0.3-prelim/data/results/t215_safe_summary.json"
