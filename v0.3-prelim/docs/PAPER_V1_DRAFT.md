...
We present a velocity-dependent self-interacting dark matter (SIDM) architecture — **a phenomenological constraint map and no-go catalogue, not a unified particle-physics model** — that addresses the tension between Cloud-9's high self-interaction requirement (σ/m ≥ 50 cm²/g at v ≈ 28 km/s) and the dwarf galaxy upper limits (σ/m ≲ 0.8 cm²/g at v ≈ 5–15 km/s). The multi-resonance + two-component + gravothermal framework describes **4 of 8 observational channels under physically motivated f_H** (Yang+ 2025-derived or T202 N-body-derived), and **7 of 8 channels only under borrowed (hand-picked) f_H** that is not self-consistently derived at Phase 44 parameters. The Cloud-9 vs dSph tension is **unresolved at Phase 44 parameters**; the framework cannot simultaneously satisfy both constraints with first-principles f_H. Path F1 (§9.9-§9.11) addresses the structural SPARC limitation via the three-term decomposition σ_eff = f_H² σ_HH + 2 f_H f_L σ_HL + f_L² σ_LL: under borrowed prescription mode, σ_eff(100) ≈ 0.19; under the priored free fit, SPARC log L ≈ −2 (z ≈ 2.0, clear fail). Path F1 is a structural fix, not an automatic data-resolution. Five UV completion no-go theorems (magnetic dipole DM, Hidden U(1) + pseudo-Dirac, GeV-scale inelastic DM, Chu+ 2019 p-wave resonance, T184 one-mediator systematic) apply to the Phase 44 single-component baseline. A two-mediator Drobczyk 2025 candidate achieves thermal relic density Ωh² = 0.119 at δ = 0.43%, but does **not** solve the Cloud-9 vs dSph tension. We document the framework as a **constraint map and no-go catalogue**, identifying the σ/m(v) regime where standard Yukawa physics breaks down.
...
**Scope (added in v18.41 per 2review.docx Reviewer 2 §1.3):** The five no-go theorems below apply specifically to the **Phase 44 single-component σ/m = 0.052 cm²/g at v = 100 km/s baseline** with standard Yukawa physics. The T163 KK-tower best-fit parameters (α_D = 0.3, m₀ = 0.3 GeV, r = 1.5, n_modes = 2, RMSE = 1.408) are a finer-grained realization within the same Phase 44 framework and have **not** been independently re-tested against the no-go theorems. Re-running the no-gos against T163 parameters is deferred to v19.0 (requires ~half day of dedicated code work). The T184 one-mediator UV systematic is qualitatively different from the other four: it is a general scaling argument rather than a specific UV construction.
