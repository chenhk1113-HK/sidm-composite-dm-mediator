...
**Our test (archived T87):** The composite-DM model at v0.7 MAP (m_χ = 770 GeV, m_φ = 453 MeV, ε ≈ 1.1×10⁻³⁷, α_χ ≈ 6.8×10⁻¹⁷) was tested against the LZ event at the time of the September 2026 announcement using the archive document `v0.3-prelim/docs/archive/other/T87_LZ_FORWARD_PREDICTION.md` (with Di Mauro cross-link in §13). Key result: the predicted N_events at the LZ event energy (248 keV assumed, δ = 297 keV tested) is **1.3×10⁻⁷⁵** — **74 orders of magnitude below the observed 1 event**. The deficit is dominated by ε² × F²_composite: at the freeze-in regime (ε ~ 10⁻³⁷), the model's direct-detection cross-section is intrinsically suppressed far below secluded-WIMP values.

**Verdict:** Composite-DM at v0.7 MAP **fails this test by 74 orders of magnitude**. This is the strongest no-go channel in our composite-DM direct-detection test, and importantly, it is **falsifiable** — any future composite-DM parameter point that recovers the LZ event rate would have to break the freeze-in suppression by 70+ orders. Per the project's cross-link to Di Mauro 2026 [51], this deficit is consistent with the magnetic-moment (T90) and Higgsino-inelastic branches being separate EFT channels; the composite-DM σ_DM-nuc is not a generic WIMP cross-section.

**Honest framing:** We do not claim the LZ event is real. We test whether, *if* it is real, our model can explain it. The answer is no, by 74 orders of magnitude. If the event is later attributed to background or revised to <2σ, this channel becomes "untestable" rather than "failed"; if it is confirmed at higher significance, this verdict stands and the composite-DM parameter space must be revisited at higher ε or with SD operators.
