...
**(b) Fresh T196 test against v18.11 posteriors (T192 Drobczyk candidate, m_χ = 10.3 GeV, g_h_SM = 0.00040, σ_SI = 2×10⁻⁴⁹ cm²):** script `v0.3-prelim/code/T196_v18_lz_event_rate.py` initially gave N_events ≈ 4.35×10⁻²⁹ (28.36 orders deficit). The corrected analysis (T199, `v0.3-prelim/code/T199_corrected_lz_analysis.py`) uses the standard Lewin-Smith 1996 elastic v_min formula `v_min = c × sqrt(m_N × E_R / (2 × μ²))` where `μ = m_χ × m_N / (m_χ + m_N)` (NOT the `m_χ²` in denominator used in T196/T197, which was a bug). At m_χ = 10.3 GeV, E_R = 5.4 keV, m_N = 131 GeV: **v_min = 591 km/s**, which IS below the SHM escape velocity + lab motion threshold of 776 km/s. Therefore v18.11 IS kinematically accessible. **T200** (`v0.3-prelim/code/T200_properly_corrected_lz.py`) further corrects the rate formula to include N_target = M_target × N_A / A_mol (T199 was missing this factor, off by ~24 orders of magnitude) and uses the standard Lewin-Smith truncated-Maxwell-Boltzmann eta function. **Final v18.11 result: N_events ≈ 1.18×10⁻³ at LZ** — a **3-order deficit below the 1-event threshold**, fully consistent with the LZ null. At DARWIN's projected exposure (200 t·y), v18.11 predicts N ≈ 10 events, which would be a detectable signal. The T187 benchmark σ_SI = 1.23×10⁻⁴⁶ at 10 GeV (cited in §10 as "RIGHT AT LZ SENSITIVITY") assumed the LZ 2023 low-E analysis (~1 keV threshold), not the 2026 extended-window analysis that produced the 248 keV event.
...
**Comparison:** Four models, four distinct verdicts (T200 with proper N_target, reduced-mass TS&W, truncated-MB eta):
- v0.7 composite-DM (T87): kinematically accessible, σ_DM_nuc too small — fails by **68 orders** at LZ (was 115 in T199 due to missing N_target factor)
- v18.11 Drobczyk (T196/T199/T200): kinematically **ACCESSIBLE** (v_min = 591 km/s < SHM threshold 776 km/s), under-predicts LZ by only **3 orders** (N ≈ 1.18×10⁻³, well below 1-event threshold)
- Di Mauro 2026 inelastic (T200): kinematically **INACCESSIBLE** (v_min = 2418 km/s with reduced-mass TS&W > 776 km/s) — 0 events at all xenon/argon detectors
- T90 magnetic-moment (T200): kinematically accessible, OVER-predicts LZ by **~7 orders** (N ≈ 7×10⁶ events at LZ with the LZ-untuned cross-section); LZ-tuned (T198 WIMpy) gives ~1 event at LZ by construction

**Honest framing (T200):** The earlier "no tested parameter point reaches LZ sensitivity" framing was correct in spirit but wrong in magnitude. With T200's corrections, v18.11 DOES reach LZ sensitivity (~3 orders short of the 1-event threshold); the v0.7 composite-DM fails by a smaller margin (68 vs 115 orders); T90 is excluded by over-prediction (a NEW failure mode not seen in T197); Di Mauro remains inaccessible.
