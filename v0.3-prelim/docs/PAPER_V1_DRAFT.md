...
**Our test (archived T87 + fresh T196):** The composite-DM model was tested against the LZ event in two regimes:

**(a) Frozen T87 test against v0.7 MAP (m_χ = 770 GeV, m_φ = 453 MeV, ε ≈ 1.1×10⁻³⁷, α_χ ≈ 6.8×10⁻¹⁷):** archived document `v0.3-prelim/docs/archive/other/T87_LZ_FORWARD_PREDICTION.md` (with Di Mauro cross-link in §13) computed the inelastic σ_DM-nuc at the LZ event energy (248 keV, δ = 297 keV tested). Result: σ_inel_nuc = 1.15×10⁻¹¹⁷ cm², **N_events ≈ 4.81×10⁻⁷³** (72.3 orders below observed 1 event). The deficit is dominated by ε² × F²_composite: at the freeze-in regime (ε ~ 10⁻³⁷), the model's direct-detection cross-section is intrinsically suppressed far below secluded-WIMP values.

**(b) Fresh T196 test against v18.11 posteriors (T192 Drobczyk candidate, m_χ = 10.3 GeV, g_h_SM = 0.00040, σ_SI = 2×10⁻⁴⁹ cm²):** script `v0.3-prelim/code/T196_v18_lz_event_rate.py` recomputes the LZ event rate using v18.11's σ_SI (a direct thermal-relic prediction from the two-mediator UV completion, not the freeze-in composite-DM σ_DM-nuc). **N_events ≈ 4.35×10⁻²⁹** (28.36 orders below observed 1 event). Result file: `v0.3-prelim/data/results/t196_v18_lz_event_rate.json`.

**Comparison:** The v18.11 Drobczyk candidate is **70 orders of magnitude closer** to LZ sensitivity than v0.7 composite-DM (28 vs 98 orders deficit), but still falls far short. The σ_SI scaling factor of 10⁻⁴⁹/10⁻¹¹⁷ ≈ 10⁶⁸ reflects the structural shift from freeze-in composite-DM (ε² × F²_composite suppression) to thermal-relic two-mediator UV completion (where σ_SI is set by relic density).

**Verdict:** Composite-DM at v0.7 MAP **fails this test by 72.3 orders of magnitude** (T87 frozen); the v18.11 Drobczyk candidate is closer but still fails by **28.4 orders** (T196 fresh). This is the strongest no-go channel in our composite-DM direct-detection test, and importantly, it is **falsifiable** — any future composite-DM parameter point that recovers the LZ event rate would have to break the freeze-in suppression by 70+ orders. Per the project's cross-link to Di Mauro 2026 [51], this deficit is consistent with the magnetic-moment (T90) and Higgsino-inelastic branches being separate EFT channels; the composite-DM σ_DM-nuc is not a generic WIMP cross-section.

**Honest framing:** We do not claim the LZ event is real. We test whether, *if* it is real, our model can explain it. The answer is **no** for both v0.7 composite-DM (fails by 72.3 orders) and v18.11 Drobczyk candidate (fails by 28.4 orders). If the event is later attributed to background or revised to <2σ, this test becomes "untestable" rather than "failed"; if it is confirmed at higher significance, both verdicts stand and the composite-DM / two-mediator parameter spaces must be revisited at higher ε or with SD operators.
