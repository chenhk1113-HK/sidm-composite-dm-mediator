...
**Status:** Paper draft (v1.10 FINAL, INTERNAL REFERENCE, **Markdown source of truth — no PDF build during drafting**) incorporating Phases 32–54. v1.10 is the submission candidate following the wayforward.docx (2026-09-19) review. It adds the honest abstract sentence about the Cloud-9 / dSph velocity lever-arm problem (no smooth σ(v) function simultaneously satisfies both constraints without fine-tuning), quotes the 25–92× tension at specific v_eff values (5, 10, 15 km/s), and cites the closed alternative-mechanism investigations (T101, T110) in §8.4. v1.10 corrects the §3.6 dSph tension magnitude from 800× (v1.9) to 25–92× after the Horigome+ 2025 velocity convention was corrected from v=30 km/s to the actual v_eff = 0.64 × V̂_max (which is ~10–20 km/s for classical dSphs and ~3–10 km/s for UFDs); the earlier v=30 km/s attribution was a misreading of the Horigome+ paper. v1.9 corrected from 38× to 800× was based on the T101.4 kinematics bug fix (E_R = ¼ m_χ v² instead of ½ m_χ v²); v1.10 supersedes v1.9's velocity convention as well. v1.9 added §8.4 "Limitations and Future Work" addressing the three actionable points from `qwen1.docx` (2026-09-17): (i) partial-wave / numerical Schrödinger treatment; (ii) hierarchical forward-model for SPARC; (iii) Boltzmann-solver relic density. v1.9 also added §8.5 "Honest mixed verdict" paragraph. v1.8 added "Limitations and Future Work". v1.7 addressed two post-review actions from `Reviewtest.docx` (2026-09-17): (i) §2.1 explicit declaration that the v²-space Breit-Wigner form is canonical, with v-space form as independent cross-check; (ii) explicit acknowledgement of v-space vs v²-space differences. v1.6 addressed four reviewer recommendations from `testreport-review.docx` (2026-09-17): (i) §2.1 v_target vs v_peak distinction (note: the v_peak ≈ 41 km/s claim was an artifact of the wrong E_R formula; v1.9 re-evaluates this); (ii) §3.6 dSph tension quantification (now corrected to 25–92× in v1.10); (iii) JVAS 24× consistent; (iv) Figure 1. T101 partial-wave evaluation (`wip/T101-partial-wave`) is documented in `T101_4_DECISION_GATE_REPORT_2026_09_19.md`; T110 alternative-mechanism investigation (`wip/RSIDM-near-threshold`, `wip/inelastic-SIDM`) is documented in `T110_1A_NEAR_THRESHOLD_RESULT_2026_09_19.md` and `T110_1B_INELASTIC_RESULT_2026_09_19.md` (both closed with negative results). **v1.10 is the final version per `wayforward.docx` review: freeze here, treat as submission candidate.**
...
**Result:** ⚠ Real tension with the multi-resonance architecture, but smaller than initially reported. The Horigome+ 2025 [27] analysis uses the velocity-dependent form σ/m = σ₀/m × F(w/v_eff) where v_eff = 0.64 × V̂_max (Eq. 12-15 of [27], following Yang & Yu 2022 [30]). For the classical dSphs in their sample (Draco, Fornax, Sculptor), V̂_max ~ 15–30 km/s, giving v_eff ~ 10–20 km/s. For UFDs (Segue 1, etc.), V̂_max ~ 5–15 km/s, giving v_eff ~ 3–10 km/s. **Note:** an earlier version of this paper (v1.6–v1.9) incorrectly applied the constraint at v = 30 km/s; this was based on a misreading of the Horigome+ convention. The correct velocity scale is v_eff ~ 10–20 km/s (classical dSphs) or v_eff ~ 3–10 km/s (UFDs).

At the correct velocity v_eff = 15 km/s, the Phase 44 free fit gives σ/m(v=15) ≈ 5.0 cm²/g, which is a factor of ~25× above the Horigome+ 2025 95% CL upper limit of 0.2 cm²/g (for velocity-independent SIDM). At v_eff = 10 km/s, σ/m ≈ 6.5 cm²/g, violation ~32×. At v_eff = 5 km/s (UFDs), σ/m ≈ 18 cm²/g, violation ~92×. The violation magnitude varies across the dSph sample because σ/m(v) is non-monotonic due to the BW tails of the v₁ resonance. The model still violates a reasonable interpretation of the Horigome+ dSph constraint by 25–90× at the correct velocities — this is a known limitation of the current phenomenological form. Reducing σ/m at v_eff = 5–20 km/s below 0.2 would require a qualitatively different functional form (e.g., near-threshold resonance per Chu, Hambye, Tytgat 2018/2019 [28, 29], or inelastic channel with mass splitting); narrowing the v₁ Breit-Wigner breaks the Cloud-9 constraint at v ≈ 28 km/s (see §2.1). This is a real physical limitation of the current phenomenological form, not a fitting problem.
