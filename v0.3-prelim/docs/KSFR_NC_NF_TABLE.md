...
|---|---|---|---|---|---|---|
| 2 | 2 | **≈ 8.0** | ±1.0 (conservative) | 400 | ESTIMATED | See §4 below; no published continuum limit for SU(2) fund Nf=2 found. |
...
### 3.5 (Nc, Nf) = (2, 2) — ESTIMATED

**Source class: ESTIMATED.**

- The case **SU(2) with Nf=2 Dirac flavours in the fundamental** is the
  composite-Higgs prototype (`SU(4)/Sp(4)` coset, 5 PNGBs, with the SM Higgs
  doublet as 4 of them). It has been studied extensively — but, importantly,
...
- These studies focus on **Sp(4)** (the gauge group that realises the
  `SU(4)/Sp(4)` coset, *not* SU(2) itself), because Sp(4) with fundamental
  matter has the same flavour symmetry breaking pattern as SU(2) with
  fundamental matter — but the **gauge group itself is different**, so the
  vector meson spectrum is not identical.
- **Crucial point from Bennett et al. (2024):** in SU(2) fundamental Nf=2 the
  dimensionless ratio `m_V / (√2 f_PS)` — the *right-hand side of the KSFR
  relation* `g_VPP = m_V / (√2 f_PS)` — is *not* in the table they print.
  They report `m_V / f_PS` values for *Sp(4)*, not *SU(2)*.
- The **only** continuum-chiral lattice value for SU(2) *fundamental* in the
  published record (and what the `t53b_lattice_input.py` references) is the
  adjoint-representation calculation: **R(2, 2, adjoint) = 6.5 ± 0.5**
  (`Athenodorou, Bennett et al., Phys. Rev. D z6bp-cckl, arXiv:2406.04233`).
  This is *not* the same physics — adjoint fermions in SU(2) realise a
  different chiral-symmetry-breaking pattern than fundamental fermions.
- **Best estimate for SU(2) fundamental Nf=2:** R should be close to the
  SU(3) value (8.4) because the leading-order large-Nc result says R is
  approximately Nc-independent. The Nc=2 SU(2) value is likely **R ≈ 7–9**,
  with central estimate **R = 8.0**.
- **Confidence: low-to-medium.** A conservative ±1.0 error bar is appropriate.
- `m_rho_MeV_min = 50 × 8.0 = 400 MeV`.
- **Action item:** see §5 — a targeted literature dig for SU(2) *fundamental*
  Nf=2 lattice spectrum values would close this gap.
...
2. **(2, 2) fundamental**: Re-read `Arthur et al., arXiv:1607.06654` and the
   Sp(4) ↔ SU(2) relation discussed in `Bennett et al., PRD 109 (2024)
   034504, arXiv:2304.01070`. If a continuum-chiral `m_V / f_PS` value for
   SU(2) fundamental Nf=2 is reported anywhere, use it.
...
- `R. Arthur, V. Drach, A. Hietanen, C. Pica, F. Sannino, "SU(2) gauge theory
  with two fundamental flavours: Scalar and pseudoscalar spectrum",
  arXiv:1607.06654`. (SU(2) Nf=2 fundamental spectrum — but I could not
  extract a continuum-chiral R value; needs follow-up.)
- `E. Bennett et al., "Sp(4) gauge theories on the lattice: Nf=2 dynamical
  fundamental fermions", JHEP 12 (2019) 053, arXiv:1909.07342`. (Sp(4), not
  SU(4), but same coset physics.)
