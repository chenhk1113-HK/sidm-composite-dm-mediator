...
**Reference:** T39 Tier-3 fit at `v0.3-prelim/data/results/t39_tier3_epsilon_alpha_joint_fit.json` shows log_Z = -2.94, MAP (σ/m_0, a, ε, α) = (-0.14, 1.31, 10⁻⁵⁶, 10⁻²⁸).

---

## Summary table — 7 missing posteriors, ranked

| # | Missing posterior | Adds dimensions | Priority | Estimated cost | Particle physics gain | Status |
|---|---|---|---|---|---|---|
...
2. Convert σ/m_0 → σ × m_χ/m_A'² to expose mass dependence
...
2. Convert σ/m_0 → coupling × g²/m_A'⁴ to expose mediator dependence
...
**2–4 weeks** of simulation work. Requires AMUSE pipeline improvement (currently N=10³, would need N≥10⁴).
...
### Phase 2 (1–2 weeks, HIGH priority): particle-physics posterior

**Add m_χ and m_A' as new dimensions** (posterior #1 + #2).
- Convert σ/m_0 → σ × m_χ/m_A'⁴
...
**Why this is the top priority**: This is the most impactful single change. Transforms the project from phenomenology to particle physics. ~2–4 weeks of focused work.

### Phase 3 (1–2 weeks, LOW priority, parallel to Phase 2): cross-channel correlations

**Add hierarchical nuisance parameters** (posterior #6).
- Identify shared systematics
- Add correlated-likelihood framework
- Re-run with hierarchical model

**Why lower priority**: Doesn't change headline answer, only uncertainty bars.

### Phase 4 (2–4 weeks, MEDIUM priority): particle-physics model class
...
### Phase 5 (4–8 weeks, MEDIUM priority): baryonic feedback

**Test SIDM vs CDM+baryonic degeneracy** (posterior #4).
- Pull EAGLE/IllustrisTNG simulations
- Compare to SPARC rotation curves
- Compute Bayes factor between models

**Why medium priority**: Could invalidate the SIDM interpretation, so it must be done before claiming "SIDM is the answer."

### Phase 6 (2–4 weeks, LOW priority): gravothermal time-evolution

**Extend AMUSE pipeline for time-dependent σ/m** (posterior #7).
- Improve AMUSE from N=10³ to N≥10⁴
...
### Phase 7 (3–6 months, LOW priority, only if Phase 6 succeeds): alternative DM classes
