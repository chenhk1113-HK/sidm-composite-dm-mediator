...
> **Status:** v18.42 (2026-09-26) — **T175 verification + T213 KK tower + Silverman+ combined test**. T175 confirms all 4 no-go verdicts hold at T163 KK-tower best-fit parameters (v18.41 §10 "T163 re-test deferred to v19.0" is now obsolete). T213 confirms T163 σ/m(V_max=31.12 km/s) = 0.174 cm²/g, which is 5.7× below the Silverman+ 2026 gravothermal threshold of 1.0 cm²/g — the KK tower is in the Born regime with flat velocity dependence (factor < 1.04 across 5-500 km/s), so it cannot bridge the Cloud-9 gap. The combined result reinforces the structural constraint map verdict: single KK tower is the wrong tool for Cloud-9 scale, Silverman+ gravothermal is the right mechanism but wrong mass scale. See [`docs/PAPER_V1_DRAFT.md`](docs/PAPER_V1_DRAFT.md) §10 for the updated scope statement, [`docs/T213_KK_TOWER_SILVERMAN_COMBINED_2026-09-26.md`](docs/T213_KK_TOWER_SILVERMAN_COMBINED_2026-09-26.md) for the full T213 doc. v18.40 T212 refinements (Path A3 + Path B3 trim) and v18.41 2review.docx response (abstract compression, headline retirement, T209 NFW sampling bug fix, T210 justifications) preserved.
