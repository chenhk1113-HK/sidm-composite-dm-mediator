...
grep -c "# @assert majorant < N -- disabled" src/DSMC.jl/src/collision.jl || echo "  NO disabled asserts (patch not applied)"
grep -c "# @assert majorant <= N -- disabled" src/DSMC.jl/src/collision.jl || echo "  NO second disabled assert (patch not applied)"
echo ""
