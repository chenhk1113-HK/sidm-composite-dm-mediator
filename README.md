...
**SIDM constraint map + no-go catalogue (v18.32, 2026-09-23): a velocity-dependent SIDM architecture (multi-resonance σ/m(v) + two-component + gravothermal) achieves **partial observational-channel coverage** — best read as a phenomenological interpolation through 8 channels, not a first-principles derivation. **v18.32 retracts the v18.31 T206 Path D "data prefer extreme segregation" finding**: T206's likelihood function was inverted for one-sided constraints, AND the σ_eff = f_H² × σ_HH(v) decomposition cannot match SPARC's σ/m ≈ 0.193 at v = 100 km/s regardless of f_H (max σ_eff = 0.069 < 0.193). The T206 fit is structurally degenerate. Paper framing reverts to v18.30: constraint map + no-go catalogue, Cloud-9 vs dSph tension unresolved at Phase 44. 4 no-go theorems rule out one-mediator UV; two-mediator Drobczyk candidate viable at δ=0.43% (requires composite UV).**

[![License: MIT](https://img.shields.io/badge/License-MIT-yellow.svg)](LICENSE)
[![Version](https://img.shields.io/badge/version-0.4--prelim%2Bv18.32-blue)](VERSION)
...
**Honest caveat (v18.32):** Earlier versions of this paper reported a "7 of 8 channels pass" headline (v18.22 and earlier), which used hand-picked f_H_at_r values from a placeholder function. After the v18.29-v18.30 Rule 28 arithmetic audit revealed the placeholder, we ran T206 (v18.31 Path C check) — fitting f_H_at_r as a free parameter. That result claimed "data prefer extreme segregation" (f_H_core_collapsed ≤ 0.07). **v18.32 retracts that finding**: the T206 likelihood function was inverted for one-sided constraints, AND the σ_eff = f_H² × σ_HH(v) decomposition cannot match SPARC's σ/m ≈ 0.193 at v = 100 km/s regardless of f_H. The T206 fit was structurally degenerate. The paper's honest framing reverts to v18.30: a constraint map + no-go catalogue, not a unified model. The Cloud-9 vs dSph tension is **unresolved** at Phase 44.
